...
import { storeResume } from './blob-storage.js';

async function candidatePayloadForTurso(form, candidateId = 'pending') {
  const payload = {};
  for (const [key, value] of form.entries()) {
    if (typeof File !== 'undefined' && value instanceof File) {
      const stored = await storeResume(value, candidateId);
      if (stored) { payload.resume = stored.filename; payload.resume_url = stored.url; }
    } else if (key !== 'resume') {
      payload[key] = value === 'true' ? 1 : value === 'false' ? 0 : value;
    }
  }
  return payload;
}
...
export async function createCandidateForWorkspace({ workspaceId, clerkUserId, data, resume }) {
  const form = candidateFormData({ workspaceId, clerkUserId, data, resume });
  const payload = process.env.TURSO_DATABASE_URL ? await candidatePayloadForTurso(form) : form;
  const candidate = await createRecord('candidates', payload);
...
export async function updateCandidateForWorkspace({ workspaceId, clerkUserId, candidateId, data, resume }) {
  ...
  const payload = process.env.TURSO_DATABASE_URL ? await candidatePayloadForTurso(form, candidateId) : form;
  const candidate = await updateRecord('candidates', candidateId, payload);
